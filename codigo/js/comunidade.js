...
function loadCommunity() {
    ...
    communityImage.src = community.imagem ? community.imagem : "https://placehold.co/400";
    editBtn.href = `cadastroComunidade.html?id=${community.id}`;
    searchBar.oninput = (e) => {searchCommunity(e.target.value)};
    bannerElement.style.setProperty("--banner-img", `url('${community.imagem ? community.imagem : "https://placehold.co/400"}')`);
}
...
function searchCommunity(term) {
    ...
    const filteredComs = communities.filter(com => com.name.toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').includes(normalizedTerm));

    communityList.innerHTML = filteredComs.map(com => 
        `<div>
