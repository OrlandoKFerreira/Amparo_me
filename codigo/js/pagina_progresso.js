...
async function iniciarPaginaInicial(containerGrid) {
    ...
    containerGrid.innerHTML = progressos.map(prog => `
...
function carregarDiario(container) {
    const textoDiario = progressos.find(prog => prog.data == diaAtualKey).diario;
    ...
    document.getElementById('btnEditDiario').addEventListener('click', () => mostrarEditorDiario(container));

    // Configura botão EXCLUIR ---
    document.getElementById('btnDeleteDiario').addEventListener('click', () => {
        // Pergunta de confirmação
        if(confirm("Tem certeza que deseja excluir este diário permanentemente?")) {
...
async function mostrarEditorDiario(container) {
    ...
    container.querySelector('.btn-cancel').addEventListener('click', () => carregarDiario(container));
}
const perguntas = [
...
function carregarHumor(container) {
    if (humores.length > 0) {
        ...
        container.innerHTML = respostas.map(resp=> `
...
function configurarAbas() {
    ...
    const defaultTab = document.querySelector('.tab-button[data-target="diario"]');
